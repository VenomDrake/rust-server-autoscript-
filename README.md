...
```bash
bash -c "$(curl -fsSL https://raw.githubusercontent.com/VenomDrake/rust-server-autoscript-/main/ct/rust-server.sh)"
```

The command above is ready to paste as-is: `OWNER` was a documentation
placeholder in an earlier version and must not appear in the URL.
The URL uses the repository owner `VenomDrake`; it will become available from
the `main` branch after this change has been merged.
...
### Troubleshooting a `404` before startup

If `curl` reports HTTP 404 and the command contains
`raw.githubusercontent.com/OWNER`, the obsolete placeholder command is being
used. Copy the command above again; do not type `OWNER` literally. Run it on
the Proxmox VE node itself, because the installer requires Proxmox's `pct`
command and cannot create an LXC from inside a regular Ubuntu VM.
